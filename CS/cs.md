...
8. 가상 머신과 도커 컨테이너의 차이
  - 가상 머신은 하드웨어 기반의 여러 게스트 운영체제로 하이퍼 바이저를 통해 에뮬레이션 됨
  - 컨테이너는 어플리케이션 수준으로 구성되고 커널 하나를 공유해서 여러가지 가상 환경을 에뮬레이션 함
  - VM은 게스트 운영체제를 구축해야 하기 때문에 오버헤드가 있지만 각각 OS도 제공할 수 있는 반면에, 컨테이너는 운영체제에 대한 오버헤드가 없는 대신 항상 같은 OS에 다른 환경을 제공함

9. Monolithic Archtecture와 Microservice Architecture
  - 모놀리틱 아키텍쳐 : 하나의 어플리케이션 내에 모든 로직이 들어가 있는 통짜 구조
    - 규모가 작은 경우에 배포나 운영이 매우 쉽고, 하나의 구조이기 때문에 관리가 쉬움
    - but, 실제로 규모가 커지면 배포나 아키텍쳐의 구동시간이 매우 오래 걸리고, 하나의 버그로 인해 어플리케이션 자체가 동작하지 않는 문제가 발생할 수 있음
  - 마이크로서비스 아키텍쳐 : 매우 작은 단위의 서비스를 여러 개가 모여 하나의 시스템을 제공하는 구조
    - 각 서비스는 느슨하게 연결되어 있으며, 어플리케이션 배포나 구동을 독립적인 마이크로 서비스별로 할 수 있다는 장점이 있음
    - but, 서비스간 통신이 필요하고, 이 통신이 지연(latency)를 만든다거나, 서비스간 통신으로 인한 테스트의 난이도가 높고, 일반적으로 복잡한 구조로 통합적인 유지 관리가 어려워짐
